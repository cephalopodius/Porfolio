...
class BackController extends Controller{
  ...
  public function connection($mail , $password){
    ...
    if ($mail || $password ) //checking void field
    {
      ...
      if ($test == true){
        $message = '<p>zetes co et session </p>
  ...
  public function addPost($titre,$chapo,$contenu,$image){
    ...
    $this->repository=new BlogRepository();
		$newBlog = new Blog($titre,$chapo, $contenu,$image,'',1,1);
  ...
  echo $message;
    ...
    public function editPost($titre,$chapo,$contenu,$image,$idBlog){
      ...
      $this->repository=new BlogRepository();
      $newBlog = new Blog($titre,$chapo, $contenu,$image,'',1,$idBlog);
    ...
    public function addUser($nom,$prenom,$mail,$password){
      ...
      $this->repository=new UserRepository();
      $user = new User('',$prenom,$nom,'',$mail,$password);
    ...
    echo $message;
      ...
      public function addComment($content,$idBlog){
        ...
        $this->repository=new ComRepository();
        $com = new Comment('',0,$content,'',$_SESSION['id_User'],$idBlog);
